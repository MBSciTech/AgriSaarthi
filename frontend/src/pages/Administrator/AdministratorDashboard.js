...
function AdministratorDashboardHeader({ name }) {
  return (
      ...
      <span className="fw-bold" style={{ fontSize:22, letterSpacing: 1, lineHeight: 1.1, marginBottom: 0 }}>
        <span style={{ display: 'inline-block', verticalAlign: 'middle', marginRight: 10 }}>
          <a href="/profile" title="View Profile" style={{ display: 'inline-block' }}>
            <img src="/images/roles/administrator.png" alt="Profile" style={{ width: 38, height: 38, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgb(9, 98, 76)', background: '#fff', cursor: 'pointer' }} />
          </a>
        </span>
        Welcome Administrator{name ? `, ${name}` : ""}!
      </span>
    </div>
  );
}

export default function AdministratorDashboard({ profile = {} }) {
  return (
    <div
      style={{
        minHeight: "100vh",
