...
export default function Market() {
        ...
        const fetchProfileAndPrices = async () => {
            ...
            try {
                ...
                const marketResponse = await fetch(
                  `http://localhost:8000/api/market-prices?state=Maharashtra&district=Nashik&commodity=Mataki`, {
                      method: "GET",
                      headers: {
    ...
    const filteredData = data
        .filter(rec => {
            // FIX: Guard against null or undefined properties before calling .toLowerCase()
            // This prevents the app from crashing if the API returns incomplete data.
            const commodity = rec.commodity || "";
            ...
            const matchesCommodity = filterCommodity === "all" ||
                commodity.toLowerCase() === filterCommodity.toLowerCase();

            return matchesSearch && matchesCommodity;
        })
        .sort((a, b) => {
            // FIX: Add guards for sorting as well to prevent similar errors.
            if (sortBy === "market") {
    ...
    const uniqueCommodities = [...new Set(data.map(rec => rec.commodity).filter(Boolean))];
    ...
    const stats = {
        ...
        bestPrice: filteredData.length > 0 ? Math.max(...filteredData.map(rec => parseFloat(rec.modal_price || 0))) : 0,
        lowestPrice: filteredData.length > 0 ? Math.min(...filteredData.map(rec => parseFloat(rec.modal_price || 0))) : 0,
        bestMarket: filteredData.length > 0 ?
            filteredData.reduce((max, rec) => (parseFloat(rec.modal_price || 0) > parseFloat(max.modal_price || 0) ? rec : max), filteredData[0]) : null,
        cheapestMarket: filteredData.length > 0 ?
            filteredData.reduce((min, rec) => (parseFloat(rec.modal_price || 0) < parseFloat(min.modal_price || 0) ? rec : min), filteredData[0]) : null
    };
